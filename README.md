...
- **Works on Legacy Non-PBIR Reports** → Retrofitted to work with the Legacy Non-PBIR Reports - Planning to also submit as a PR to Semantic Link Labs core repo.
-  **Workspace Selector** → Only want to run this against 1, 2, 10 workspaces? Enter any number of workspaces to run against. 'All' will still run against eveyrthing and a built-in timer ensures no selection will run against everything.
